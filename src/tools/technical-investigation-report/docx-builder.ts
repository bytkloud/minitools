...
function titleBlock() {
  return new Paragraph({
    children: [run('TECHNICAL INVESTIGATION REPORT', { bold: true, size: 32 })],
...
function buildObservationBlock(obs: ObservationData, index: number): (Paragraph | Table)[] {
  const items: (Paragraph | Table)[] = [
    new Paragraph({
      children: [run(`${index + 1}.  ${obs.text}`, { size: 20 })],
      spacing: { before: 120, after: 80 },
    }),
  ]

  const photos = obs.photos.filter(Boolean) as string[]
  if (photos.length > 0) {
...
export async function downloadDocx(
  data: ReportData,
  filename = 'technical-investigation-report.docx',
) {
  const obsChildren = data.observations.flatMap((obs, i) => buildObservationBlock(obs, i))

  const doc = new Document({
    creator: 'Technical Investigation Report Tool',
    styles: {
      default: {
        document: {
    ...
    sections: [{
      ...
      children: [
        ...
        sectionHeading('Description of the Accident [as per the Driver / Customer]'),
        bodyParagraph(data.accidentDescription),

        sectionHeading('Observations and Comments'),
        ...obsChildren,

        sectionHeading('Recommendation'),
        bodyParagraph(data.recommendation),
